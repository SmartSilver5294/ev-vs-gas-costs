...
```{r}
# Predict cost difference from gas and EV prices
linear_reg() |> 
    fit(cost_diff ~ Regular + Cost_per_kWh, data = combined_data)
```
...
$$
\begin{equation}
\text{Cost Difference}_i = \beta_0 + \beta_1 \times \text{Regular}_i + \beta_2 \times \text{Cost per kWh}_i + \epsilon_i
\end{equation}
$$
