...
kable(vehicle_choices, 
      ...
      col.names = c("Index", "Vehicle Comparison (EV vs Gas)", "EV Efficiency", "Gas Efficiency")) %>%
...
create_summary_data <- function() {
  map_dfr(1:nrow(ev_vs_gas_cars), function(i) {
    ...
    tibble(
      ...
      `Avg Savings per Year` = paste0("$", format(round(mean(costs$model_cost_diff), 0), big.mark = ",")),
      `States w/ Savings` = sum(costs$model_cost_diff > 0),
      `States w/ Losses` = sum(costs$model_cost_diff < 0)
    )
  }) %>%
    arrange(desc(parse_number(`Avg Savings per Year`)))
}
```
...
datatable(summary_data, 
  ...
  formatStyle(columns = "Avg Savings per Year", 
